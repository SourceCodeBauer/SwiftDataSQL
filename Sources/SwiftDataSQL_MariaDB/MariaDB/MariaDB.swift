...
public final class MySQL {
    ...
    static nonisolated(unsafe) private var dispatchOnce = pthread_once_t()
    ...
    private static nonisolated(unsafe) var initOnce: Bool = {
  ...
  public func ping() -> Bool {
	...
	public func connect(host hst: String? = nil, user: String? = nil, password: String? = nil, db: String? = nil, port: UInt32 = 0, socket: String? = nil, flag: UInt = 0) -> Bool {
        ...
        #if os(macOS)
        var ssl_mode = 0; // 0 = no usar SSL
        mysql_options(self.ptr!, MYSQL_OPT_SSL_ENFORCE, &ssl_mode);
        #endif
        #if os(macOS)
//        var SSL_MODE_DISABLED: UInt32 = 1 // Valor estándar de MariaDB para "disabled"
//        mysql_options(self.ptr!, MYSQL_OPT_SSL_MODE, &SSL_MODE_DISABLED)
        #endif
        #if os(macOS)
            let empty = UnsafePointer<CChar>(bitPattern: 0) // NULL
            mysql_options(self.ptr!, MYSQL_OPT_SSL_KEY, empty)
            mysql_options(self.ptr!, MYSQL_OPT_SSL_CERT, empty)
            mysql_options(self.ptr!, MYSQL_OPT_SSL_CA, empty)
            mysql_options(self.ptr!, MYSQL_OPT_SSL_CAPATH, empty)
            mysql_options(self.ptr!, MYSQL_OPT_SSL_CIPHER, empty)
        #endif
