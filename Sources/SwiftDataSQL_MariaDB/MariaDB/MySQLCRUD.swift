...
class MySQLCRUDRowReader<K : CodingKey>: KeyedDecodingContainerProtocol {
	...
	init(_ db: MySQL,
		...
		columns = cols
		row = r
        var collectedKeys: [Key] = []
        for columnNameString in self.columns.keys { // Itera sobre las claves de tu mapa `columns`
            if let keyInstance = Key(stringValue: columnNameString) {
                collectedKeys.append(keyInstance)
            } else {
                // Opcional: Manejar el caso en que un nombre de columna no pueda ser una Key
                // Esto es raro para ColumnKey si el stringValue es válido.
                print("Advertencia: No se pudo crear una CodingKey para el nombre de columna: \(columnNameString)")
            }
        }
        self.allKeys = collectedKeys
	}
	...
	func decode<T>(_ type: T.Type, forKey key: Key) throws -> T where T : Decodable {
        ...
        default:
            fatalError("Invalid type \(special)")
		}
	}
